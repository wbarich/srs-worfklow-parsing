# 1. Introdution
This repository is about generating **Workflows** (graphs) from a set of **System Requirments Specification (SRS) data**. 
...
2. **Generation**: By designing a suitable prompt, an LLM is used to create graphs from the SRS data. GPT3.5 Turbo is used in this repository. Although other LLMs may be used for this task, the OpenAI API is used because of its ease of use and speed. 
3. **Evaluation**: The graphs generated by the LLM are compared against the hand-annotations to determine the efficacy of the approach. The results are interrogated and improvements are listed.


# 2. Data Preparation
A huge amount of effort must be spent in creating a suitable training data set for this project. Understanding what the SRS data means took at least 30% of the time in the development of this work. For this reason the number of hand annotated samples are limited (4 Projects which is about 75 nodes; see notes on *Use of Terminlogy* below).

Several samples are hand annotated in this repository. In the hand annotation process crucial quetsion arise: 'What is the purpose of a workflow?' and 'What are the characteristics of a well defined workflow?' The answer that I came up with is that a Workflow describes how a system operates from start to finish. In other words if you look at a diagram of the workflow you should be able to fairly easily understand what the system is about from the perspective of how a user would use it. I also decided that the granularity of the Workflows should not be too granular, as this would make the diagrams too intricate and difficult to follow. However should this solution be used for serious applications, these design changes may be changed. 

In the chosen dataset, each set of SRS data is allocated to a specific project. So ProjectID 1 has a set of system requirements, Project 2 has a seperate set of system requirements. The term 'ProjectID' is used extensively throught this work to refer to a separate set of system requirements.
...
2. Further training data. To better evaluate the efficacy of the system much much more training data needs to be created.
